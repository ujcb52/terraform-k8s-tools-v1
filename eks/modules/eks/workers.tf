...
resource "aws_launch_configuration" "workers" {
  count       = var.create_eks ? local.worker_group_count : 0
  name = "${aws_eks_cluster.this[0].name}-${lookup(var.worker_groups[count.index], "name", count.index)}"
...
resource "aws_security_group" "workers" {
  ...
  name = "${var.cluster_name}-${data.aws_caller_identity.current.account_id}-worker-sg"
